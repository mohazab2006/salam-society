...
export default function ContactSection() {
  ...
  function validate() {
    ...
    if (!form.name.trim()) newErrors.name = c.nameRequired;
    if (!form.email.trim()) newErrors.email = c.emailRequired;
    else if (!/\S+@\S+\.\S+/.test(form.email)) newErrors.email = c.emailInvalid;
    if (!form.message.trim()) newErrors.message = c.messageRequired;
    else if (form.message.trim().length < 10) newErrors.message = c.messageTooShort;
    return newErrors;
  }
  ...
  const inputCls = (hasError: boolean) =>
    `w-full px-4 py-3 rounded-xl border text-gray-900 text-sm placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-brand/30 focus:border-orange-brand transition-colors ${
        ...
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 lg:gap-20 items-start">
            ...
            >
              ...
              >
                {c.title}{" "}
                <span className="text-orange-brand">{c.titleHighlight}</span>
              </h2>
              <p className="text-white/50 text-base leading-relaxed mb-8">{c.subtitle}</p>
              ...
              <div className="space-y-5">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-white/40 text-xs font-500 uppercase tracking-wide mb-0.5">{c.emailLabel}</p>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-white/40 text-xs font-500 uppercase tracking-wide mb-0.5">{c.locationLabel}</p>
              ...
              <div className="flex items-center gap-3 mt-8">
                <a href="https://www.instagram.com/salamsociety.ca/" target="_blank" rel="noopener noreferrer" aria-label="Instagram"
                ...
                <a href="https://www.youtube.com/@SalamSociety" target="_blank" rel="noopener noreferrer" aria-label="YouTube"
                  className="w-10 h-10 rounded-full border border-white/15 flex items-center justify-center text-white/50 hover:text-white hover:border-orange-brand transition-all">
                ...
                <a href="https://chat.whatsapp.com/Bdwcs9euaZI3UnfKDrhtHL" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp"
                  className="w-10 h-10 rounded-full border border-white/15 flex items-center justify-center text-white/50 hover:text-white hover:border-[#25D366] hover:bg-[#25D366]/10 transition-all">
              ...
              <div className="bg-white rounded-3xl p-8 shadow-[0_20px_60px_rgba(0,0,0,0.3)]">
                {state === "success" ? (
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-xl font-700 text-gray-900 mb-2">{c.successTitle}</h3>
                    <p className="text-gray-500 text-sm mb-6">{c.successSub}</p>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                    <div>
                      <label htmlFor="name" className="block text-sm font-600 text-gray-700 mb-1.5">{c.nameLabel}</label>
                      <input id="name" name="name" type="text" value={form.name} onChange={handleChange}
                        placeholder={c.namePlaceholder} className={inputCls(!!errors.name)} />
                      {errors.name && <p className="text-red-500 text-xs mt-1">{errors.name}</p>}
                    </div>

                    <div>
                      <label htmlFor="email" className="block text-sm font-600 text-gray-700 mb-1.5">{c.emailFieldLabel}</label>
                      <input id="email" name="email" type="email" value={form.email} onChange={handleChange}
                        placeholder={c.emailPlaceholder} className={inputCls(!!errors.email)} />
                      {errors.email && <p className="text-red-500 text-xs mt-1">{errors.email}</p>}
                    </div>

                    <div>
                      <label htmlFor="message" className="block text-sm font-600 text-gray-700 mb-1.5">{c.messageLabel}</label>
                    ...
                    {state === "error" && (
                      <p className="text-red-500 text-sm bg-red-50 px-4 py-3 rounded-xl">{c.errorMsg}</p>
                    )}
