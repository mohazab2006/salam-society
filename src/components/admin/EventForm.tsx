    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-soft border border-gray-100 space-y-4">
        ...
        <div>
          ...
          <input name="slug" value={form.slug} onChange={handleChange} required placeholder="event-slug" className={inputClass} />
          <p className="text-xs text-gray-400 mt-1">URL: /events/{form.slug || "..."}</p>
        </div>

        <div>
          <label className={labelClass}>Description *</label>
          <textarea name="description" value={form.description} onChange={handleChange} required rows={5} placeholder="Describe the event..." className={inputClass} />
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-soft border border-gray-100 space-y-4">
        ...
        <div>
          ...
          <select name="audience_category" value={form.audience_category} onChange={handleChange} className={inputClass}>
        ...
        <div>
          ...
          <input name="age_group" value={form.age_group} onChange={handleChange} placeholder="e.g. 6–11, 13+, All ages" className={inputClass} />
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-soft border border-gray-100 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="time" name="end_time" value={form.end_time} onChange={handleChange} className={inputClass} />
          </div>
        </div>

        <div>
          <label className={labelClass}>Location *</label>
          <input name="location" value={form.location} onChange={handleChange} required placeholder="Venue name, Ottawa, ON" className={inputClass} />
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-soft border border-gray-100 space-y-4">
        ...
        <label className="flex items-center gap-3 cursor-pointer">
          <input type="checkbox" name="signup_required" checked={form.signup_required} onChange={handleChange} className="w-4 h-4 rounded border-gray-300 accent-orange-brand" />
        ...
        {form.signup_required && (
          <div>
            ...
            <input name="signup_link" value={form.signup_link} onChange={handleChange} type="url" placeholder="https://..." className={inputClass} />
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-soft border border-gray-100 space-y-4">
        ...
        <div className="flex flex-col gap-3">
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" name="featured" checked={form.featured} onChange={handleChange} className="w-4 h-4 rounded border-gray-300 accent-orange-brand" />
            <span className="text-sm font-500 text-gray-700">Feature on homepage</span>
          </label>
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" name="published" checked={form.published} onChange={handleChange} className="w-4 h-4 rounded border-gray-300 accent-orange-brand" />
